...
def run_command(
    cmd: list, capture_output: bool = True, check: bool = True
...
def check_prerequisites() -> None:
    ...
    if not Path("Taskfile.yml").exists():
        print_colored(
            "✗ Please run this script from the repository root directory", Colors.RED
        )
        sys.exit(1)
    ...
    try:
        ...
        run_command(
            [
                "bash",
                "-c",
                "source ../../scripts/set-aws-credentials.sh && export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY",
            ],
        ...
        result = run_command(
            [
                "bash",
                "-c",
                "source ../../scripts/set-aws-credentials.sh && export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY && mise exec -- tofu output -raw postgresql_backup_access_key_id",
            ]
        )
        ...
        result = run_command(
            [
                "bash",
                "-c",
                "source ../../scripts/set-aws-credentials.sh && export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY && mise exec -- tofu output -raw postgresql_backup_secret_access_key",
            ]
        )
...
def check_1password_item_exists(
    op_cmd: list, item_name: str, vault: str, account: str
) -> bool:
    """Check if a 1Password item exists"""
    try:
        run_command(
            op_cmd + ["item", "get", item_name, "--vault", vault, "--account", account]
        )
        return True
    except subprocess.CalledProcessError:
        return False
    ...
    try:
        run_command(
            ...
            + [
                ...
                f"password={secret_access_key}",
                "--tags",
                "aws,postgresql,s3,backup,database",
...
def main():
    ...
    if check_1password_item_exists(op_cmd, item_name, vault, op_account):
        ...
        if update_1password_item(
            op_cmd, item_name, vault, op_account, access_key_id, secret_access_key
        ):
            print_colored(
            ...
            print_colored(
                "✗ Failed to update item. You may need to update manually.", Colors.RED
            )
        ...
        if create_1password_item(
            op_cmd, item_name, vault, op_account, access_key_id, secret_access_key
        ):
            print_colored("✓ Successfully created new item in 1Password!", Colors.GREEN)
        else:
            print_colored(
                "✗ Failed to create item. Please create manually with:", Colors.RED
            )
    ...
    print_colored(
        "   export AWS_SECRET_ACCESS_KEY='[Retrieved from 1Password]'", Colors.BLUE
    )

...
class OnePasswordStorage:
    ...
    def create_item(
        self, config: CredentialConfig, credentials: AWSCredentials
    ) -> bool:
...
class CredentialManager:
    ...
    def __init__(self, provider: CredentialProvider, storage: OnePasswordStorage):
    ...
    def update_credentials(self, config: CredentialConfig) -> bool:
        ...
        self.print_colored(
            f"Updating {config.service_name} Credentials in 1Password", Colors.BLUE
        )
        ...
        try:
            ...
            self.print_colored(f"✗ Failed to retrieve credentials: {e}", Colors.RED)
            return False

        self.print_colored("✓ Retrieved new credentials", Colors.GREEN)
        self.print_colored(f"  Access Key ID: {credentials.access_key_id}", Colors.BLUE)

        # Store in 1Password
        self.print_colored("\nChecking if 1Password item exists...", Colors.YELLOW)

        if self.storage.item_exists(
            config.onepassword_item_title, config.onepassword_vault
        ):
            self.print_colored(
                f"✓ Found existing item: {config.onepassword_item_title}", Colors.GREEN
            )
            ...
            if self.storage.update_item(config, credentials):
                self.print_colored(
                    "✓ Successfully updated credentials in 1Password!", Colors.GREEN
                )
            else:
                self.print_colored(
                    "✗ Failed to update item. Manual update required.", Colors.RED
                )
                self._print_manual_instructions(config, credentials)
                return False
        else:
            self.print_colored("! Item not found. Creating new item...", Colors.YELLOW)

            if self.storage.create_item(config, credentials):
                self.print_colored(
                    "✓ Successfully created new item in 1Password!", Colors.GREEN
                )
            else:
                self.print_colored(
                    "✗ Failed to create item. Manual creation required.", Colors.RED
                )
                self._print_manual_instructions(config, credentials)
                return False
    ...
    def _print_success_message(
        ...
        self.print_colored("\n" + "=" * 44, Colors.GREEN)
        self.print_colored("✓ Credentials updated in 1Password!", Colors.GREEN)
        self.print_colored("=" * 44, Colors.GREEN)
        ...
        if config.s3_bucket_name:
            ...
            self.print_colored(
                "   export AWS_SECRET_ACCESS_KEY='[From 1Password]'", Colors.BLUE
            )
            self.print_colored(
                f"   export S3_BUCKET='{config.s3_bucket_name}'", Colors.BLUE
            )
            self.print_colored(
...
def check_prerequisites() -> None:
    ...
    if not Path("Taskfile.yml").exists():
        raise RuntimeError("Please run this script from the repository root directory")
